...
class FocalLoss(nn.Module):
    """Focal Loss with label smoothing and γ-smoothing compensation.
    
    Key interaction: Label smoothing (ε) dampens focal loss's focusing effect.
    Smoothing prevents overconfident predictions, so p_t stays lower → focal term
    (1 - p_t)^γ is uniformly larger → easy examples get less down-weighting.
    
    With ε=0.1 and K=7 classes, the smoothed target caps at ~0.914, which means
    p_t plateaus lower during training. The effective focusing is equivalent to
    running γ_effective ≈ γ * (1 - ε(K-1)/K) without smoothing.
    
    To compensate, we optionally scale γ upward: γ_adjusted = γ / (1 - ε(K-1)/K).
    Set compensate_smoothing=True to enable this correction.
    """
    def __init__(self, alpha=None, gamma=2.0, reduction='mean', label_smoothing=0.0,
                 num_classes=7, compensate_smoothing=True):
        super(FocalLoss, self).__init__()
        self.alpha = alpha
        self.reduction = reduction
        self.label_smoothing = label_smoothing
        
        # Compensate for smoothing dampening of focal effect
        if compensate_smoothing and label_smoothing > 0:
            dampening_factor = 1.0 - label_smoothing * (num_classes - 1) / num_classes
            self.gamma = gamma / dampening_factor
            print(f"[FocalLoss] γ-smoothing compensation: γ={gamma} → γ_adjusted={self.gamma:.3f} "
                  f"(ε={label_smoothing}, K={num_classes}, dampening={dampening_factor:.4f})")
        else:
            self.gamma = gamma
    ...
    def forward(self, inputs, targets):
        ...
        # This isolates the model's true confidence from the smoothing regularization
...
# Keep every sample each epoch to avoid dropping tail examples.
KEEP_INCOMPLETE_BATCH = True  # True -> drop_last=False

# Stronger regularization to improve generalization.
MODEL_DROP_RATE = 0.35
MODEL_DROP_PATH_RATE = 0.20

# Accuracy-focused checkpoint selection.
BEST_MODEL_SELECTION = "blended"  # options: "accuracy", "f1", "blended"
BEST_ACC_WEIGHT = 0.7
BEST_F1_WEIGHT = 0.3

# Focal Loss hyperparameters
FOCAL_GAMMA = 2.0  # Focusing parameter (2.0 is standard)
LABEL_SMOOTHING = 0.05  # Lower smoothing tends to help top-1 accuracy

# Offline balancing
AUG_DIR = "./data/augmented"  # Root for pre-generated augmented images
# NOTE: Run 02_balance_dataset.py BEFORE training to generate balanced datasets
...
def main():
    ...
    print(f"Dropout: {MODEL_DROP_RATE} | DropPath: {MODEL_DROP_PATH_RATE}")
    print(f"Best model selection: {BEST_MODEL_SELECTION} (acc={BEST_ACC_WEIGHT}, f1={BEST_F1_WEIGHT})")
    print("="*50)
    ...
    for fold in range(N_FOLDS):
        ...
        print(f"Original Train: {len(train_df)} images | Val: {len(val_df)} images")

        # Load pre-balanced training CSV (generated by 02_balance_dataset.py)
        balanced_csv = os.path.join(DATA_DIR, f"balanced_fold{fold}.csv")
        if not os.path.exists(balanced_csv):
            raise FileNotFoundError(
                f"Balanced dataset not found: {balanced_csv}\n"
                f"Run 'python 02_balance_dataset.py' first!"
            )
        balanced_train_df = pd.read_csv(balanced_csv)
        fold_aug_dir = os.path.join(AUG_DIR, f"fold{fold}")
        
        print(f"Balanced Train: {len(balanced_train_df)} images (includes augmented)")
        ...
        # === AUGMENTATION & BALANCING SUMMARY ===
        ...
        # Count original vs augmented per class
        train_labels = [label_map_sampler[row['dx']] for _, row in balanced_train_df.iterrows()]
        class_counts_balanced = np.bincount(train_labels, minlength=7)
        n_original = len(balanced_train_df[balanced_train_df['is_augmented'] == False])
        n_augmented = len(balanced_train_df[balanced_train_df['is_augmented'] == True])
        
        print(f"\n{'='*60}")
        print(f"OFFLINE BALANCING SUMMARY (Fold {fold + 1})")
        print(f"{'='*60}")
        print(f"Original images: {n_original}")
        print(f"Augmented images: {n_augmented}")
        print(f"Total balanced set: {len(balanced_train_df)}")
        print(f"\nBalanced class distribution:")
        for cls_idx in range(7):
            name = SHORT_NAMES[cls_idx]
            count = class_counts_balanced[cls_idx]
            # Count how many are augmented for this class
            cls_name = idx_to_class[cls_idx]
            n_aug_cls = len(balanced_train_df[(balanced_train_df['dx'] == cls_name) & (balanced_train_df['is_augmented'] == True)])
            print(f"  [{cls_idx}] {name:15s}: {count:5d} ({n_aug_cls} augmented)")
        print(f"{'='*60}\n")
        ...
        train_loader = DataLoader(
            ...
            shuffle=True,  # Simple shuffle for the pre-generated offline dataset
            num_workers=NUM_WORKERS, 
            pin_memory=True, 
            persistent_workers=use_persistent_workers,
            drop_last=not KEEP_INCOMPLETE_BATCH
        )
        ...
        # NOTE: Class weights are currently not used in Focal Loss.
        # Offline augmentation already addresses class imbalance to a large extent,
        # and focal gamma keeps emphasis on harder examples.
        ...
        # compensate_smoothing=True adjusts γ upward to counteract label smoothing dampening
        criterion = FocalLoss(alpha=None, gamma=FOCAL_GAMMA, label_smoothing=LABEL_SMOOTHING,
                              num_classes=7, compensate_smoothing=True)
        print(f"Using Focal Loss with γ={FOCAL_GAMMA} (compensated), label_smoothing={LABEL_SMOOTHING}\n")
        ...
        if os.path.exists(resume_path):
            ...
            best_fold_f1 = checkpoint.get('best_f1', 0.0)
            best_fold_acc = checkpoint.get('best_acc', checkpoint.get('val_acc', 0.0))
            best_fold_score = checkpoint.get('best_score', best_fold_f1)
            ...
            print(f"Best so far -> F1: {best_fold_f1:.4f} | Acc: {best_fold_acc:.4f} | Score: {best_fold_score:.4f}\n")
        ...
        for epoch in range(start_epoch, EPOCHS):
            ...
            if BEST_MODEL_SELECTION == "accuracy":
                selection_score = val_acc
            elif BEST_MODEL_SELECTION == "f1":
                selection_score = val_f1
            elif BEST_MODEL_SELECTION == "blended":
                selection_score = BEST_ACC_WEIGHT * val_acc + BEST_F1_WEIGHT * val_f1
            else:
                raise ValueError(f"Unsupported BEST_MODEL_SELECTION: {BEST_MODEL_SELECTION}")
            ...
            print(f"Epoch {epoch + 1}/{EPOCHS} | {duration:.1f}s | "
                f"Train Loss: {epoch_loss:.4f} | Val Acc: {val_acc:.4f} | Val F1: {val_f1:.4f} | Score: {selection_score:.4f} | "
            ...
            if selection_score > best_fold_score:
                best_fold_f1 = val_f1
                best_fold_acc = val_acc
                best_fold_score = selection_score
                ...
                checkpoint = {
                    ...
                    'config': {
                        ...
                        'best_acc_weight': BEST_ACC_WEIGHT,
                        'best_f1_weight': BEST_F1_WEIGHT,
                    }
                }
                torch.save(checkpoint, save_path)
                print(
                    f"   Saved best model: {save_name} "
                    f"(Acc: {best_fold_acc:.4f}, F1: {best_fold_f1:.4f}, Score: {best_fold_score:.4f})"
                )
                
                # NEW: Send webhook for new best
                send_curl_log({
                    "content": f"**NEW BEST - Fold {fold + 1}**\nEpoch {epoch + 1} | Acc: {best_fold_acc:.4f} | F1: {best_fold_f1:.4f}"
                })
        ...
        print(f"\n Fold {fold + 1} complete! Best Acc: {best_fold_acc:.4f} | Best F1: {best_fold_f1:.4f} | Best Score: {best_fold_score:.4f}")
        send_curl_log({
            "content": f" **Fold {fold + 1} Complete** | Best Acc: {best_fold_acc:.4f} | Best F1: {best_fold_f1:.4f}"
        })
        print("-" * 50)
